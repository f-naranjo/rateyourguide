...
export default class PageGuidesBook extends Component {
    _isMounted = false;
    ...
    componentDidMount() {
       if(this.props.location.filterParams){
        const {location,language,people,dateFrom,dateTo} = this.props.location.filterParams
        this._isMounted = true;
        this.tourService.filterGuidesBook(location,language,people,dateFrom,dateTo)
            .then((guides) => {
                console.log(guides)
                if (this._isMounted) {
                    this.setState({
